...
This gem is a re-write of the APN for Rails gem written by PRX and Mark Bates which is also a re-write from a plugin that was written by Fabien Penso and Sam Soffes.
APN on Rails is a good gem but it has its limitations and restrictions which doesn't allow Mongoid love.

Most of the documentation listed below is a duplication of the documentation by APN on Rails. I respect all the work that went into that gem.
...
<pre><code>
  $ rails console
  >> d = APN::Device.create(:udid => "123456abcdefg", :token => "XXXXXXXX XXXXXXXX XXXXXXXX XXXXXXXX XXXXXXXX XXXXXXXX XXXXXXXX XXXXXXXX")
